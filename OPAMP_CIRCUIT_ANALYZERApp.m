classdef OPAMP_CIRCUIT_ANALYZERApp < handle
    %
    % Usage:
    %     app.OPAMP_CIRCUIT_ANALYZERApp
    ...
    properties (Constant)
        AppPathBase = {'/OPAMP_CIRCUIT_ANALYZER'};
        AppClass = 'OPAMP_CIRCUIT_ANALYZERApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function obj = OPAMP_CIRCUIT_ANALYZERApp()
        ...
        function startApp(obj)
            ...
            mlock;
            OPAMP_CIRCUIT_ANALYZERApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'FinalApp')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong FinalApp
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'FinalApp').getString, ...
            ...
            if nargout(@FinalApp) == 0
                eval('FinalApp');
            else
                obj.AppHandle = eval('FinalApp');
            end
